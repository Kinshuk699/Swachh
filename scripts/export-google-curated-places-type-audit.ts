...
function toMarkdown(input: { generatedDate: string; inputPath: string; rows: AuditRow[] }): string {
  ...
  return [
    ...
    "## Rubric Summary",
    "",
    "- Dhaba in the resolved Google name stays candidate-only and is not recommended for public-map promotion by default.",
    "- Road, bridge, route, toll, and area objects are recommended for removal.",
    "- Premium/official fuel and wayside rows become Tier 1 only when Google types and resolved names support the premium/highway format.",
    "- Generic fuel stations become Tier 3 fallback stops and must not inherit premium fuel labels.",
    "- Known organized restaurant, cafe, food-plaza, and tourism stops become Tier 2 when they are not dhabas.",
    "- Lodging/resort rows stay candidate-only unless separately verified.",
    "",
    "## Counts",
    "",
